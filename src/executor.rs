...
impl Executor {
    ...
    pub unsafe fn run_task(&self, ctx: &mut LlamaContext, task: SwarmMessage) -> Option<SwarmMessage> {
        if let SwarmMessage::ProcessTask { task_id, input_state, start_layer: _, end_layer: _, tokens: _ } = task {
