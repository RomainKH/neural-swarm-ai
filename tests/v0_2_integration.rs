use neural_swarm_ai::{Orchestrator, Executor, SwarmMessage, InferenceBackend};
...
async fn test_v0_2_full_pipeline_flow() -> Result<()> {
    ...
    let p1 = NodeProfile::custom(neural_swarm_ai::compute::profile::DeviceType::Desktop, 8, 8000, "node1".into());
    let p2 = NodeProfile::custom(neural_swarm_ai::compute::profile::DeviceType::Desktop, 8, 8000, "node2".into());
    ...
    let (node1_id, task1) = orchestrator.start_sequence(1, "task1".into(), vec![42], Bytes::from(initial_data.clone()))?.unwrap();
